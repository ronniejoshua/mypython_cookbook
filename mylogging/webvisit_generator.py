...
file_handler = logging.FileHandler('webvisit_generator.log')
file_handler.setFormatter(formatter)
logger.addHandler(file_handler)

# logging.basicConfig(level=logging.INFO, filename='webvisit_generator.log', format=FORMAT, datefmt='%d-%b-%y %H:%M:%S')
...
class IPGenerator(object):
    ...
    @staticmethod
    def random_ip():
        octets = [str(random.randrange(223) + 1)]
        for i in range(3):
...
class LogGenerator(object):
    ...
    def write(self, dest, count):
        for i in range(count):
            ...
            request = urllib.parse.quote(
                request.replace("*PRODUCT*", str(product)).replace("*DEPARTMENT*", dept).replace("*CATEGORY*",
                                                                                                 cat).lower())
            ...
            # Hard-coded as Python has no standard timezone implementation
            date = datetime.now().strftime("%d/%b/%Y:%H:%M:%S -0800")
            dest.write(
                "%(ip)s - - [%(date)s] \"GET %(request)s HTTP/1.1\" %(resp_code)s %(resp_size)s \"-\" \"%(ua)s\"\n" %
    ...
    def pick_weighted_key(hash):
        ...
        for (key, weight) in hash.items():
            if running <= rand < (running + weight):
                return key
...
logger.info("Stop data generation")
